      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-2 justify-center text-foreground mb-4">
        <section>
          ...
          <div className="flex flex-col items-center sm:items-start">
            {links.map((link) => (
              <NextLink as={Link} key={link.title} href={link.href} className="text-sm text-foreground/50 hover:text-foreground">
                {link.title}
              </NextLink>
            ))}
          </div>
        ...
        <section className="col-span-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4">
            {categories.map((category) => (
              <NextLink as={Link} key={category.id} href={`/products/${category.attributes.slug}`} className="text-sm text-foreground/50 hover:text-foreground justify-center sm:justify-start">
