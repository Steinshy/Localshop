...
const Breadcrumb: FC<BreadcrumbProps> = ({ requestUrl }) => {
  ...
  const buildItems = useCallback((): BreadcrumbItems => {
    ...
    for (let i = 0; i < urls.length; i++) {
      ...
      const title = capitalize(unslug(urls[i])),
        url = buildHref(urls, i),
          ...
          {items.map((item, index) => (
            <>
              {item.href ? (
                ...
                <span key={`breadcrumb_${index}`} className="text-small font-bold text-foreground/50 truncate text-ellipsis">
                  {item.title}
                </span>
              )}
