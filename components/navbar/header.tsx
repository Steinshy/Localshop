    ...
    <div className="relative">
      <Navbar isBlurred={false} isBordered isMenuOpen={isMobileMenuOpen} onMenuOpenChange={setIsMobileMenuOpen}>
        ...
        <NavbarContent className="flex sm:hidden" justify="start">
          ...
          <MobileMenu isMobileMenuOpen={isMobileMenuOpen} handleMobileMenu={handleMobileMenu} navItems={navItems} isItemActive={isItemActive} />
        </NavbarContent>
        ...
        <NavbarContent justify="end">
          {isLogged() && (
            <NavbarItem>
              <Badge content={cartTotal} color="danger" placement="top-right" variant="shadow" isInvisible={cartTotal <= 0}>
                <Button startContent={<FaCartArrowDown className="text-2xl" />} as={Link} href="/order" size="md" variant="ghost" radius="md" className="hidden md:flex">
                  Cart
                </Button>
                <Button isIconOnly as={Link} href="/order" size="md" variant="ghost" radius="sm" className="flex md:hidden">
      ...
      {/* CategoriesMenu */}
      <div className={`${isCategoriesMenuOpen ? 'translate-y-0 opacity-1' : '-translate-y-full opacity-0 invisible'} z-[39] top-[65px] transition-all duration-500 ease-in-out absolute bg-background shadow-lg w-full flex justify-center p-2`} onMouseLeave={() => handleCategoriesMenu(false)}>
