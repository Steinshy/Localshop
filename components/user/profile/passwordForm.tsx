        ...
        <Formik
          ...
          onSubmit={(values: PasswordValuesProps, { setSubmitting, setFieldError }) => {
            const apiFetch = async () => {
              ...
              if (!error) {
                Object.keys(values).forEach((key) => {
                  values[key as keyof PasswordValuesProps] = '';
                });
          ...
          {({ errors, isSubmitting }) => (
            <Form className="flex flex-col gap-2">
              <div className="grid grid-cols-1 gap-2">
                <Field label="New Password" id="password" name="password" type="text" as={Input} radius="sm" size="sm" isDisabled={isSubmitting} isInvalid={errors.password} color={errors.password ? 'danger' : 'default'} errorMessage={errors.password && errors.password} />
