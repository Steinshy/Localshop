...
const OrderCard: FC<OrderCardProps> = ({ order, detailed = false }) => {
  const { attributes: { id, total, createdAt, totalItems, status, items, shipping, billing } } = order;
    ...
    <Card className='border-2 w-full'>
      ...
      <CardBody>
        ...
        <div className={detailed ? 'grid grid-cols-1 gap-2 mt-2' : 'flex gap-2 mt-2 p-1'}>
          {items.map((item) => <OrderProductCard key={`product_${id}`} item={item} detailed={detailed} />)}
        </div>
      </CardBody>
    </Card>
  );
