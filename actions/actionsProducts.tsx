...
export const getBestSellers = async () => {
  ...
  try {
    const { data } = await api.get<{ data: ProductResponse[] }>('/products/bestsellers', { next: { tags: ['products'] } });
    return { data };
  } catch (e) {
...
export const getNewest = async () => {
  ...
  try {
    const { data } = await api.get<{ data: ProductResponse[] }>('/products/newest', { next: { tags: ['products'] } });
    return { data };
  } catch (e) {
    const error = handleError(e as Error | ErrorObj | string);
    return { data: [] as ProductResponse[], error };
...
export const getLastPieces = async () => {
  ...
  try {
    const { data } = await api.get<{ data: ProductResponse[] }>('/products/last_pieces', { next: { tags: ['products'] } });
    return { data };
  } catch (e) {
    const error = handleError(e as Error | ErrorObj | string);
    return { data: [] as ProductResponse[], error };
...
export const getProducts = async (page?: number, query?: string, category?: string) => {
  ...
  try {
    const { products, pagy } = await api.get<{ products: { data: ProductResponse[] }; pagy: PagyProps }>(`/products?page=${page}&q=${query}&category=${category}`, { next: { tags: ['products'] } });
    const { data } = products;
    return { data, pagy };
  } catch (e) {
    const error = handleError(e as Error | ErrorObj | string);
