...
export const userLogin = async () => {
  try {
    const response = await api.get<string>('/user_login');
...
export const updateProfile = async (profileData: ProfileValuesProps) => {
  try {
    const { data } = await api.patch<{ data: UserResponse }>('/user/update_profile', JSON.stringify({ user: profileData }));
    return { data };
  } catch (e) {
    const error = handleError(e as Error | ErrorObj | string);
    return { error };
...
export const updatePassword = async (passwordData: PasswordValuesProps) => {
  try {
    const { data } = await api.patch<{ data: UserResponse }>('/user/update_password', JSON.stringify({ user: passwordData }));
    return { data };
  } catch (e) {
    const error = handleError(e as Error | ErrorObj | string);
    return { error };
